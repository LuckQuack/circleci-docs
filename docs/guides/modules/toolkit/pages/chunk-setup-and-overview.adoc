...
* Ensure you have the CircleCI GitHub App installed in your organization. Check menu:Organization Settings[VCS Connections]. Chunk needs the GitHub App installed to be able to recommend fixes and open pull requests.
...
Clicking a button opens Chunk with the failure context already filled in.
...
NOTE: **GitHub only.** These buttons are available for GitHub App organizations and standalone organizations using GitHub as their VCS. They are not available for Bitbucket or GitLab organizations.
...
|**Fix error (Beta)**
|In the step output header on the job page, next to **Explain error**.
...
|**Fix workflow (Beta)**
|On hover on a failed pipeline row in the pipeline list, to the left of `...`.
...
Each button pre-fills Chunk with the relevant failure context for that level.
For example, **Fix job (Beta)** includes all failed steps and their error logs.
**Fix workflow (Beta)** includes the workflow ID, the list of failed jobs, and the execution sequence.
