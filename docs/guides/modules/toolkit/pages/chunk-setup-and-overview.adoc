...
|**Fix error (Beta)**
|Next to the **Explain this error** option in the step output section of a failed job.

|**Fix step (Beta)**
|On hover, next to a failed step in the job view.

|**Fix job (Beta)**
|In the job header when a job has at least one failed step.

|**Fix workflow (Beta)**
|In the workflow header when a workflow has at least one failed job.
|===
...
. Click any fix button on a failed build, job, workflow, or step.
